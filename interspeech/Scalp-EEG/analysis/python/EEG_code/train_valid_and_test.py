...
from model import CNN_baseline,stanet
import tqdm
import torch
...
# ---------------------if I3D, train and valid the model_2D-----------
        ...
        for epoch in range(cfg.epoch_num):
            ...
            if valid_loss_min>valid_loss / iter:
                ...
                savedir = f'./model_2D/{cfg.decision_window}_{cfg.dataset_name}/sb' + str(sb)
    ...
    for epoch in range(cfg.epoch_num*3):
        ...
        if valid_loss_min > valid_loss / iter:
            valid_loss_min = valid_loss / iter
            savedir = f'./model_3D/{cfg.decision_window}_{cfg.dataset_name}_{cfg.model_name}/sb' + str(sb)
            if not os.path.exists(savedir):
                os.makedirs(savedir)
            saveckpt = savedir + '/fold' + str(fold) + '.ckpt'
