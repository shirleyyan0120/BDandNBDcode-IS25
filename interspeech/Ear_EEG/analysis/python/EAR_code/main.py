...
parser.add_argument('--dataset', type=str, default='pNB_EAR',choices=['B_EAR','NB_EAR','pNB_EAR'])
parser.add_argument('--decision_window', type=float, default=0.5)
parser.add_argument('--model',type = str,default='STANet',choices=['CNN_baseline','CNN_baseline','STANet'])
...
# read the data
eegname = cfg.process_data_dir + '/' +  args.dataset + '.mat'
eegdata = h5py.File(eegname, 'r')
try:
    data = from_mat_to_tensor(eegdata['EEG'])
except:
    data = from_mat_to_tensor(eegdata['EAR'])
# eeg data
...
for sb in range(cfg.sbnum):
    ...
    for fold in range(5):
        ...
        print(fold)
        eegdata = eegdata.reshape(40, datasize[1], datasize[2])
        eeglabel = eeglabel.reshape(40, datasize[1])
        ...
        traindata = traindata.reshape(24 * int(60 * cfg.sample_rate / cfg.decision_window), cfg.decision_window, 20)
        trainlabel = trainlabel.reshape(24 * int(60 * cfg.sample_rate / cfg.decision_window), cfg.decision_window)
        validdata = validdata.reshape(8 * int(60 * cfg.sample_rate / cfg.decision_window), cfg.decision_window, 20)
